...
");
  ...
  {
      // sanitize!
      $semester0 = htmlentities($semester0);
      $semester1 = htmlentities($semester1);
      print("<option value='$semester0' ".( $_POST['semester'] == $semester0 && $_POST['semester'] != NULL ? "SELECTED" : "").">$semester1</option>");
  }
 ...
 while( $class = mysql_fetch_row($query) )
 {
  ...
  {
   $info0 = htmlspecialchars($info[0]);
   print("<tr class='".( $row%2==0 ? "even" : "odd" )."'>
   <td><a class='items' href=\"JavaScript:document.classes.selectclass.value=$class[0];document.classes.page2.value=1;document.classes.submit();\" onMouseover=\"window.status='View Information For $info0';return true;\" onMouseout=\"window.status='';return true;\">$info0</a></td>
   </tr>");
  }
 }
 }
